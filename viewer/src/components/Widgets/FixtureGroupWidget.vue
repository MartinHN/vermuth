...
export default class FixtureGroupWidget extends Vue {
  mounted() {
    this.cachedPS = this.presetableState;
  }
  @Prop({ required: true })
  ...
  @Watch("expanded")
  onExpand() {
    const toCache = this.presetableState;
    const ps = this.cachedPS;
    this.$emit("presetableState:update", ps);
    this.cachedPS = toCache;
  }
  ...
  expandIfSub() {
    // if we load a preset modifying inner values, we force expand
    this.expandIfNeeded();
  }
