<template>
 ...
 -->
    <svg
      ...
      width="100%"
      height="10%"
      :viewBox="[0, 0, zoomBoxSize.w, zoomBoxSize.h]"
      @mousedown="startZoom"
    ...
    <svg
      ref="my-svg"
      width="100%"
      :height="isZoomable ? '85%' : '100%'"
      :viewBox="[0, 0, domSize.w, domSize.h]"
      @mousedown="mouseDown"
      @mouseup="mouseUp"
      @mousemove="mouseMove"
    ...
    >
      <circle
        v-for="v in draggableKeyPoints"
        :key="v.id"
        ref="framesCircles"
        :r="pRadius"
        :cx="v.location.x"
        :cy="v.location.y"
        :fill="v.hovered ? 'blue' : v.selected ? 'red' : 'black'"
      ></circle>

      <!-- <path :d=drawnPath stroke=black fill=transparent stroke-width=3></path> -->
      <path
        v-for="v in displayedKeyFramePairs"
        :key="v.id"
        :d="buildSegment(v)"
        stroke="black"
        fill="transparent"
...
-->
      <template v-for="d in draggableHandles">
        <circle
          ...
          :cy="d.location.y"
          r="6"
          :fill="d.hovered ? 'blue' : 'transparent'"
          stroke="blue"
        ></circle>

        <line
          :x1="posToPix(d.jsObj.relatedKf.position)"
          :y1="valToPix(d.jsObj.relatedKf.value)"
          :x2="d.location.x"
          :y2="d.location.y"
          stroke="blue"
        ></line>
...
function secondaryEvent(e: MouseEvent) {
  return e.metaKey;
}
...
class HandleJsType {
  constructor(
    public isB: boolean,
    public startKf: KeyFrame<number>,
    public endKf: KeyFrame<number>
  ) {}
  get relatedKf() {
    return this.isB ? this.endKf : this.startKf;
  }
}
...
export default class CurveEditor extends Vue {
  get domSize(): Size {
    ...
    return new Size(
      hasMountedDOM ? dom.clientWidth : 10,
      hasMountedDOM ? dom.clientHeight : 10
    );
  }

  public dH = new DraggableHandler();
  get displayedKeyFrames(): Array<KeyFrame<number>> {
    return this.curve.frames.filter((v) =>
      this.positionRange.inRange(v.position)
    );
  }
  ...
  get zoomPath() {
    ...
    return CurveUtils.buildCurvePath(
      this.curve,
      fullPosRange,
      fullValRange,
      reducedDomSize
  ...
  get curValuePath() {
    return [
      "M",
      0,
      this.valToPix(this.curve.getValueAt(this.position) as number),
      "H",
      this.domSize.w,
    ].join(" ");
  }
  ...
  private svgDOM: SVGGraphicsElement = {
    clientWidth: 10,
    clientHeight: 10,
  ...
  get draggableKeyPoints(): Draggable[] {
    return this.displayedKeyFrames.map((k: KeyFrame<number>): Draggable => {
      const d = new Draggable(new KeyFrameStartPointJsType(k));
    ...
    if (curHasBezier || prevHasBezier) {
      ...
      if (prevHasBezier && prevK) {
        ...
        Object.defineProperty(ld, "location", {
          ...
          set: (p: Point) => {
            setHandlePosition(p, handleType);
          },
        });
        res.push(ld);
      }
  ...
  get hoverableKeyFramePath() {
    return this.displayedKeyFramePairs.map(
      (l: [KeyFrame<number>, KeyFrame<number>]) => {
  ...
  public buildSegment(pair: [KeyFrame<number>, KeyFrame<number>]) {
    return CurveUtils.buildPathForFrames(
      pair[0],
      pair[1],
      this.positionRange,
      this.valueRange,
  ...
  public mounted() {
    ...
    if (this.curve) {
      this.positionRange.end = Math.min(
        this.curve.span,
        this.positionRange.end
      );
    }
  }

  public pixToPosVal(p: Point, absolutePoint = true) {
    return CurveUtils.pixToPosVal(
      p,
      this.domSize,
      this.positionRange,
      this.valueRange,
  ...
  public posValToPix(pv: KeyFrame<number>, absolutePoint = true) {
    return CurveUtils.posValToPix(
      new Point(pv.position, pv.value),
      this.domSize,
      this.positionRange,
      this.valueRange
    );
  ...
  public wheel(e: WheelEvent) {
    ...
    this.setZoomPosRange(
      this.positionRange.start + incZoom / 2,
      this.positionRange.end - incZoom / 2
    );
  }
  public startZoom(e: MouseEvent) {
    ...
    const doZoomMouse = (ee: MouseEvent) => {
      ...
      this.setZoomPosRange(
        zoomStartPosRange.start + inc / 2 + dx,
        zoomStartPosRange.end - inc / 2 + dx
      );
    };
    // @ts-ignore
  ...
  public setZoomPosRange(s: number, e: number) {
    ...
    this.positionRange.end = Math.max(
      this.positionRange.start + 1,
      Math.min(this.curve.span, e)
    );
  }
