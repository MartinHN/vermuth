...
if (dbgTree.enabled) {
  treeEvents.on('v', (parent: any, key: string) => {dbgTree('>>> v', key)});
  treeEvents.on(
      'move',
      (parent: any, key: string, toKey: string) => {dbgTree('>>> move', key)});
  treeEvents.on('add', (parent: any, key: string) => {dbgTree('>>> add', key)});
  treeEvents.on('rm', (parent: any, key: string) => {dbgTree('>>> rm', key)});

  treeEvents.on(
      'call',
      (parent: any, options: any, ctx: any) => {
          // dbgTree('>>> call',parent)
      });
}
...
class MessageQueue<T> {
  ...
  constructor(
      private cb: (addr: string, msg: T, id?: string) => void,
      public canCollapse: boolean, time: number, maxWait?: number) {
    this.notifyFlush = debounce(
        this.flush.bind(this), time  // 50
        ,
  ...
  public push(addr: string, args: T, collapse?: boolean) {
    ...
    const hadMessages = this.queue[addr]  //&& this.queue[addr].length
      ...
      if (hadMessages) {
        if (!addr.endsWith('_time')) {
          if (!isEqual(this.queue[addr][0].msg, params.msg)) {
            dbgMsg(`overriding msg ${addr} from ${this.queue[addr][0].msg} to ${
                params.msg}`)
          } else {
    ...
    // else {
    //   const existingIdx = this.queue[addr].findIndex(m => isEqual(m.msg,
    //   params.msg)) if (existingIdx < 0) {
    //     this.queue[addr].push(params)
    //   }
    //   else {
    //     if (!addr.endsWith('_time')) dbgMsg(`overriding msg ${addr} from
    //     ${this.queue[addr][existingIdx].msg} to ${params.msg}`)
  ...
  public flush() {
    ...
    //   assert(false,'weiiiiiird', AccessibleSettedByServer,
    //   AccessibleNotifierId)
...
const serverToClientsQueue =
    new MessageQueue(broadcastMessageFromServer, false, 30, 30);  // 50 ,100
...
export const rebuildChildAccessibles =
    (fromParent?: any, buildTypeTree = true, rebuild = true) => {
          ...
          if (buildTypeTree) {
            ...
            for (const l of Object.keys(o.__registerRemoteValuesAddr)) {
              if (o.__accessibleTypes[l] === undefined) {
                ...
                curNode[valSymbol].push(
                    {v: l, type: o.__accessibleTypes[l].name, curValue: o[l]});
              }
            }
          }
          if (rebuild) {
        ...
        if (o && o.__registerFunctionAddr) {
          if (buildTypeTree) {
            ...
            for (const [l, f] of Object.entries(o.__registerFunctionAddr)) {
              curNode[funSymbol].push({
                v: l,
                f,
                params: getFunctionParams(f as (...args: any[]) => any),
                type: o.__accessibleTypes[l].name
              });
            }
          }
          if (rebuild) {
      ...
      dbgStruct(
          'updated addr', rS.__isRoot ? 'root' : rS?.__accessibleName,
          accessibleTree);  // Array.from(allListeners.keys()));
      if (buildTypeTree) {
        return accessibleTree
      }
    };

const rebuildChildAccessiblesDebounced = debounce(
...
export function callAnyAccessibleFromRemote(
    root: any, saddr: string, args: any[],
    notifierId: string) {  // args is passed as array
  if (!assert(root, 'root not set, can\'t call')) {
    return;
  }
  if (saddr[0] === '/') {
...
export function buildAddressFromObj(o: any, errorIfEmpty = true) {
  ...
  if (insp && !insp[isProxySymbol] && insp.__accessibleParent) {
    // debugger
    if (Object.keys(insp.__accessibleParent.__accessibleMembers)
            .includes(insp.__accessibleName)) {
  ...
  while (insp && !insp.__isRoot && (maxDepth--) > 0) {
    ...
    if (insp.__accessibleName) {
      ...
      for (const [k, v] of Object.entries(
               insp.__accessibleParent.__accessibleMembers)) {
        if (v === insp) {
          idx = k;
        }
      }
      if (idx === 'null') {
        assert(false, 'not found parent accessible in array');
      }
      addr.push('' + idx);
      found = true;
      ...
      const pair =
          Object.entries(insp.__accessibleParent).find(([k, v]) => v === insp);
      if (pair) {
        addr.push(pair[0]);
        found = true;
      } else {
...
function defineOnInstance(
    targetClass: any, key: string|symbol,
    onInstance: (o: any, defaultV: any) => any) {
  const trapDepth = 0;
  let value: any;
  ...
  function clearTrap(ob: any) {
    const {enumerable: wasEnumerable} =
        Reflect.getOwnPropertyDescriptor(ob, key) || {enumerable: true};
  ...
  return (target: any, key: string|symbol) => {
    ...
    defineOnInstance(
        ...
        (parent: any, defaultValue: any) => {
          return setChildAccessible(
              parent, key, {...opts, immediate: false, defaultValue});
        },
    );
  };
}
...
function addRemoteValue(
    parent: any, k: string|symbol, defaultValue: any,
    cb?: (parent: any, value: any) => void, type?: any) {
  if ((!type || type === Object) && defaultValue !== undefined &&
  ...
  const updateAccessibleAddress =
      (deferIfUnattached = true, ncb?: () => void) => {
  ...
  Reflect.defineProperty(parent, k, {
    ...
    set: (v: any) => {
      ...
      const isSetByServer = AccessibleSettedByServer !== null &&
          registeredAddr === AccessibleSettedByServer;

      // updateAccessibleAddress(true, (hasChanged && lockCallbacks === 0 &&
      // !isSetByServer) ? doNotify : undefined);
      if (hasChanged && lockCallbacks === 0 && (!isClient || !isSetByServer) &&
        ...
        if (updateAccessibleAddress(false) !==
            false) {  // if registred adress has changed , thus not in sync
          debugger
        }
        doNotify()
      }
  ...
  defineObjTable(parent, '__registerRemoteValuesAddr')[k] =
      updateAccessibleAddress;
...
function removeRemoteValue(parent: any, key: string|symbol) {
  for (const t
           of ['__fetch', '__accessibleTypes', '__remoteValues',
               '__registerRemoteValuesAddr', '__remoteCBs']) {
    if (parent[t]) {
      delete parent[t][key]
    }
  }
...
const pruneArg =
    (e: any, options?: {allowRawObj?: boolean; noRef?: boolean}): any => {
      if (Array.isArray(e)) {
        ...
        if (!(options?.noRef === true)) {
          ...
          if (addr) {
            dbgMsg('setting ref addr as arg');
            return '/?' + addr;
          }
...
export function RemoteFunction(options?: {
  skipClientApply?: boolean;
  sharedFunction?: boolean;
  allowRawObj?: boolean;
  noRef?: boolean;
}) {
  return (target: any, key: string|symbol, descriptor: PropertyDescriptor) => {
    ...
    defineObjTable(target, '__accessibleTypes')[key] = {
      ...
      rType: Reflect.getMetadata('design:returntype', target, key) ||
          {name: 'undefined'},
      get name() {
        return `(${this.pTypes.map((e: any) => e.name)})=>${this.rType.name}`;
      },
      // undefined
    };

    const fl = defineObjTable(target, '__registerFunctionAddr')[key] =
        (parent: any) => {
          adUp.updateAddr(parent, key, false)
        };
    const rf = defineObjTable(target, '__remoteFunctions')[key] = method;
    descriptor.value = function(
        ...args: any[]) {  // need plain function for this
      // target.notifyRemote()
      let res: any;
      ...
      // call locally
      if (options && options.skipClientApply) {
        lockSkipClient += 1;
      }  // if(lockCallbacks>0){debugger}}

      if (!options || !(isClient && options.skipClientApply)) {
        // debugger
        if (options && options.sharedFunction) {
          lockCallbacks += 1;
        }  // if(lockSkipClient>0){debugger}}
          ...
          args = args.map(
              (e) => {
                if (typeof (e) === 'string' && e.startsWith('/?')) {
                  ...
                  const {accessible, parent, key: acc_key} =
                      resolveAccessible(rootAccessible, addr);
        ...
        treeEvents.emit(
            'call', this, options, {isFromShared: lockCallbacks > 0});
        if (options && options.sharedFunction) {
          lockCallbacks -= 1;
        }
        if (options && options.skipClientApply) {
...
let rootAccessible: any =
    require('./RootState');  // importing here means we need to take care at
                             // importing it first in other modules
function getRoot(hint?: any) {
  if (rootAccessible &&
      rootAccessible.__esModule) {  // once require loop ended rootAccessible is
                                    // the default export
...
type ChildAccOptions = {
  onChange?: (parent: any, el: any, valueName?: string) => void;
  immediate?: boolean;
  defaultValue?: any;
  readonly?: boolean;
  autoAddRemoteValue?: boolean;
  blockRecursion?: boolean;
  noRef?: boolean;
};
export function setChildAccessible(
    parent: any, key: string|symbol, opts?: ChildAccOptions) {
  if (reentrancyLock) {
    return parent[key];
  }
  ...
  if (typeof (childAcc) === 'object' && childAcc !== null) {
    ...
    if (childAcc.__accessibleParent && childAcc.__accessibleParent !== parent) {
      console.error(
          key, 'overriding parent', childAcc.__accessibleParent, ' with ',
          parent);
      debugger
    }
    ...
    if (childAcc[isProxyfiable]) {
      ...
      for (const [k, v] of Object.entries(childAcc)) {
        if (!isHiddenMember(k, childAcc))
          if ((v === null || typeof v !== 'object' ||
               !(v as any)[isProxySymbol])) {
            if ((!childAcc.__remoteValues ||
                 childAcc.__remoteValues[k] === undefined)) {
      ...
      if (childAcc.__onProxyfication) {
        childAcc.__onProxyfication();
      }
    ...
    const oldDesc = Reflect.getOwnPropertyDescriptor(parent, key) ||
        {};  // || { value: childAcc, configurable: false };


    // use a setter / getter to prevent accidental overwriting Proxy,
    // usually hapen in constructor
    if (oldDesc?.writable === false) readonly = true
      const desc = {
        get() {
          if (oldDesc.get) {
            oldDesc.get()  // reactive getter triggers vue dependency
                           // mechanism
          }
          return childAcc
        },
        set(v: any) {
          ...
          // }
          // if (oldDesc.set) {
          // oldDesc.set(v) // Vue reactive setter seems to be called
          // any way...
          // }
          if (v[isProxySymbol]) {
            if (!(accessibleChildBeingBuilt &&
            ...
            updateChildAccessible(
                childAcc, v, parent[key].__isConstructed !== false,
                opts)  // do sync to new object but do not send as it
                       // should happen only in constructor
          }
    ...
    if (opts?.onChange) {
      ...
      Object.defineProperty(childAcc, '__changeCB', {
        value: (el: any, valueName?: string) => {
          doSharedFunction(() => {cCB(parent, el, valueName)})
        },
        enumerable: false,
        writable: true
      });
  ...
  // parent.__accessibleMembers[k] = parent[k] ||
  // parent.__accessibleMembers[k];
  if (immediate) {
...
function removeChildAccessible(
    target: any, k: string|symbol, opts?: {doNotDelete?: boolean}) {
  treeEvents.emit('rm', target, k);
  if (target.__accessibleMembers) {
    delete target.__accessibleMembers[k];
  }
  if (target.__accessibleTypes) {
...
function updateChildAccessible(
    childAcc: any, ob: any, doSend: boolean, opts?: ChildAccOptions) {
  if (!dbg.assert(childAcc && childAcc[isProxySymbol], 'updating non proxy') ||
  ...
  for (const [k, v] of Object.entries(ob)) {
    ...
    if (!currentMembers.includes(k)) {
      if (typeof v === 'object') {
        setChildAccessible(
            childAcc, k, {...opts, immediate: true, defaultValue: v})
      ...
      if (typeof v === 'object') {
        ...
        doSharedFunction(
            () => {
                childAcc[k] = v  // should call appropriate setter but avoid
                                 // notifying change
            })
      }
    }
...
let notifiedAccessible: any =
    undefined  // used to aggregate changes when rebuilding
function notifyAccessibleChange(childAcc: any, valueName?: string) {
  if (notifiedAccessible === childAcc && valueName) {
    return
  }
  let insp = childAcc
  while (insp) {
    if (insp.__changeCB) {
      insp.__changeCB(childAcc, valueName)
    }
    insp = insp.__accessibleParent
  }
}
...
function generateAccessibleHandler(opts?: {
  blockRecursion?: boolean;
  autoAddRemoteValue?: boolean;
  noRef?: boolean;
}) {
    ...
    set(obj: any, prop: symbol|string, value: any, thisProxy: any) {
      ...
      if (Array.isArray(obj) && !isNaN(prop as any)) {
        if (value && value.__accessibleName !== undefined) {
          if (obj &&
              (obj === value.__accessibleParent ||
               thisProxy === value.__accessibleParent)) {
            if ((obj as any).__accessibleMembers) {
              if (!Object.keys((obj as any).__accessibleMembers)
                       .includes(value.__accessibleName)) {
              ...
              if (siblings && siblings[prop] &&
                  siblings[prop].__accessibleName) {
      ...
      const isNotARemoteValue = !obj.__remoteValues ||
          !Object.keys(obj.__remoteValues).includes(prop as string)
        ...
        if (typeof (prop) === 'string') {
          if (typeof (value) === 'object') {
            if (
                isNotARemoteValue
                // && !wasChildAccessible
                //  && (obj.__accessibleMembers[prop] !== newAcc ||
                //  obj.__accessibleMembers[prop].__accessibleName ===
                //  undefined) )
            ) {
              if (wasChildAccessible) {
                dbgStruct(
                    `auto update child Accessible ${prop.toString()} on `,
                    buildAddressFromObj(obj));
                updateChildAccessible(
                    obj[prop], value, !!opts?.autoAddRemoteValue, opts)
              } else {
                dbgStruct(
                    `auto add child Accessible ${prop.toString()} on `,
                    buildAddressFromObj(
                        obj));  // newAcc,
                                // Object.keys(obj.__accessibleMembers));
                setChildAccessible(
                    thisProxy || obj, prop,
                    {...opts, immediate: true, defaultValue: newAcc});
              }
            }
          } else if (opts?.autoAddRemoteValue) {
      ...
      // typeof(value)!=="object") {
      //   if(!obj.__remoteValues ||
      //   !Object.keys(obj.__remoteValues).includes(prop.toString())){
      //     addRemoteValue(obj,prop,value)
      //     const adUp = new AddressUpdater()
    ...
    get: (target: any, k: symbol|string, thisProxy: any) => {
      ...
      if (k === isProxySymbol) {
        ...
        return buildAddressFromObj(target);
          ...
          typeof ((target as any)[k]) === 'function') {
...
function extendClass<T extends Constructable<any>>(BaseClass: T):
    typeof DerivedClass {
...
export function AccessibleClass() {
  return function<I, T extends Constructable<I|unknown>>(Bconstructor: T) {
    ...
    // @ts-ignore
    // return (...args:any[]):I=> {const r = new Bconstructor(...args);(r as
    // any).isConstructed=true ; return r;}
    ...
    return class AClass extends Bconstructor {
      ...
      __dispose() {
        ...
        if ((Bconstructor as any).__dispose) {
          (Bconstructor as any).__dispose.call(this);
        }
        if ((this as any).__isConstructed) {
          Object.defineProperty(
              this, '__isConstructed',
      ...
      __undispose() {
        ...
        if ((Bconstructor as any).__undispose) {
          (Bconstructor as any).__undispose.call(this);
        }
        if (!(this as any).__isConstructed) {
          Object.defineProperty(
              this, '__isConstructed',
...
export class AutoAccessible {
  ...
  constructor() {
    ...
    return setChildAccessible(fakeParent, 'unknown', {
      defaultValue: this
    })  // new
        // Proxy(this,generateAccessibleHandler({blockRecursion:false,autoAddRemoteValue:true}))
  }
}
...
export function resolveAccessible(parent: any, addr: string[]) {
  ...
  if (inspA) {
    ...
    if (addr.length === 0) {
      ...
      if (insp) {
        const regAddr =
            (insp.getRegisteredAddress && insp.getRegisteredAddress()) || '';
        if (regAddr && addr.join('/') !== regAddr) {
          console.error(
              'accessing a non updated accessible from', addr, ' to ', regAddr)
        }
      }
...
export class RemoteMap<T> {
  constructor(
      defaultValue: any,
      private genEl: (...args: any[]) => T |
          undefined = (o: any) => {return o}) {
    if (defaultValue)
      Object.entries(defaultValue).map(([k, v]) => {this.setAtKey(k, v)})
...
export class RemoteArray<T> {
  ...
  indexOf(a: T): number{return this.l.indexOf(a)}
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () => {
        if (counter < this.l.length) {
          return {
            value: this.l[counter++], done: false
          }
        } else {
          return {
...
export function toRefString(o: any) {
  if (typeof o === 'string') {
    if (o.startsWith('/')) {
      ...
      console.error('wrong ref string')
    }
...
export function resolveRefString(s: string) {
  ...
  const {accessible, parent, key: acc_key} =
      resolveAccessible(rootAccessible, addrList);
  return accessible
}

export class Ref<T> {
  @nonEnumerable() private addr = 'unknown';
  constructor(_a = 'unknown') {
    if (typeof _a !== 'string') {
      console.error('trying to set ref from non string')
      debugger;
    }
    this.addr = _a;
  }
